...
呈清晰 U 形：过小（1e-4）欠拟合，甜区（1e-3 ~ 3e-3）达到约 1.32，过大（1e-2、1e-1）发散、loss 不降反升。发散 run（1e-1）在数千步内 loss 冲高且无法收敛，满足题面"必须包含发散 run"的要求。

**4.2 Batch Size 扫描（token 总量恒定）：**

| batch size | steps | final val loss |
|---|---|---|
| 32 | 40,000 | 1.381 |
| 64 | 20,000 | 1.438 |
| 128 | 10,000 | 1.507 |

token 预算相同时，batch 越大 → 更新步数越少 → 最终 loss 越差。小 batch（更多更新步）在此小模型上收敛更好。题面示例配置 batch=128 实际略微未达标（1.507），而 baseline 的 batch=32 达标（1.381），体现了大 batch 每步梯度更准但更新次数不足的 trade-off。
