...
- 作业题面版本：26.0.4
- 完成范围：BPE tokenizer、Transformer LM、训练工具、checkpoint、生成脚本、TinyStories/OWT 数据编码与训练、学习率 sweep、batch size sweep、四个消融、日志和图表。
...
- `scripts/train_lm.py`：读取 JSON config，训练、验证、保存 checkpoint 和 sample。
...
`3e-4` 收敛偏慢；`1e-3` 稳定；`3e-3` 在本模型上没有发散，最终 loss 最低。说明当前
10K TinyStories 任务仍处在较小模型、较短训练的 regime，适度增大学习率能更快优化。
...
下降；代价是墙钟时间也增加。若要严格比较 batch 本身，需要固定总 token budget 或加入 gradient
accumulation，本实验按作业配置记录现象。
