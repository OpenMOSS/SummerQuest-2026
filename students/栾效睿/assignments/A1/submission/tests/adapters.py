...
def run_scaled_dot_product_attention(
    ...
    return scaled_dot_product_attention(Q=Q, K=K, V=V, masked=mask)
...
def run_multihead_self_attention(
    ...
    """
    from cs336_basics.causal_multiHead_selfAttention import CausalMultiHeadSelfAttention

    multi_selfAtten = CausalMultiHeadSelfAttention(num_heads=num_heads, d_model=d_model,use_rope=False)
    state_dict = {
    ...
    multi_selfAtten.load_state_dict(state_dict)
    return multi_selfAtten(x=in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    multi_selfAtten = CausalMultiHeadSelfAttention(
        num_heads=num_heads, max_seq_len=max_seq_len, theta=theta, d_model=d_model
    )
    state_dict = {
        "q_proj.weight": q_proj_weight,
        "k_proj.weight": k_proj_weight,
    ...
    multi_selfAtten.load_state_dict(state_dict)
    return multi_selfAtten(x=in_features, token_positions=token_positions)
...
def run_rope(
    ...
    rope = RoPE(d_k=d_k, max_seq_len=max_seq_len, theta=theta)
...
def run_transformer_block(
    ...
    return trans_block(x=in_features)
...
def run_get_batch(
    ...
    return data_loading(context_length=context_length, batch_size=batch_size, device=device, token_ids=dataset)
...
def run_cross_entropy(
    ...
    return cross_entropy(output=inputs, seq=targets)
...
def run_gradient_clipping(
    ...
    grad_clip(parameters, M=max_l2_norm)
