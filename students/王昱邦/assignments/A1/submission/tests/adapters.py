...
def run_linear(
    ...
    module = Linear(in_features=d_in, out_features=d_out, dtype=weights.dtype)
...
def run_swiglu(
    ...
    module = SwiGLU(d_model=d_model, d_ff=d_ff, dtype=w1_weight.dtype)
...
def run_transformer_block(
    ...
    return module(in_features, token_positions=positions)
...
def run_rmsnorm(
    ...
    """
    module = RMSNorm(d_model=d_model, eps=eps, dtype=weights.dtype)
    module.load_state_dict({"weight": weights})
    return module(in_features)
