...
def run_swiglu(
    ...
    layer.load_weights(w1_weight, w2_weight, w3_weight)
    return layer(in_features)
...
def run_multihead_self_attention(
    ...
    layer.load_weights(q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight)
    return layer(in_features)
...
def run_get_batch(
    ...
    x = torch.stack([torch.tensor(dataset[s : s + context_length], dtype=torch.long) for s in starts])
    y = torch.stack([torch.tensor(dataset[s + 1 : s + context_length + 1], dtype=torch.long) for s in starts])
...
def run_cross_entropy(
    ...
    """
    # Numerically stable cross-entropy: logsumexp(logits) - logits[target]
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    total_norm_sq = sum(p.grad.norm(2).item() ** 2 for p in parameters if p.grad is not None)
