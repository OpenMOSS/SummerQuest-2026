# A1 学习记录 04：TinyStories 实验里程碑

- 日期：2026-07-16
- 状态：TinyStories 全部实验与 OpenWebText 32K tokenizer 已完成；OpenWebText 编码、LM 训练与跨语料 tokenizer 对比进行中

## 1. 本阶段结论

这一阶段把此前通过单元测试的 tokenizer、Transformer、训练循环和生成器接成了真实实验链路。完整 TinyStories tokenizer 在资源限制内训练完成，训练集和验证集均已编码；17M 级 Transformer 处理 327,680,000 个 token 后取得 `1.355541` validation loss，达到题目要求的 `1.45`；学习率扫、batch size 扫和文本生成也已完成。

当前结果已经证明主实现能够在真实语料上收敛。四类架构消融的六份对照实验也已经完成并回收；AdamW 资源核算与 OpenWebText 32K tokenizer 已经补齐，剩余主线是 OpenWebText 编码、训练、生成和跨语料 tokenizer 对比。
...
17 个 run 的逐点 `metrics.jsonl`、17 份可移植配置和四张 SVG loss curve 均已回收到公开提交。图由 `scripts/plot_experiment_logs.py` 从原始 JSONL 生成；机器可读汇总见 `experiments/summary.json`。

后续主线按依赖顺序为：

```text
[x] 训练 OpenWebText 32K tokenizer
└── 完成双 tokenizer / 跨语料 compression 实验
    └── 编码 OpenWebText train / validation
        └── 训练并生成 OpenWebText LM
```
