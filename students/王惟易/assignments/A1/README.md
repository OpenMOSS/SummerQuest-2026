...
- 已完成：21 个 adapter 对应实现、训练与生成入口、TinyStories 10K 与 OpenWebText 32K tokenizer、TinyStories 数据编码与完整 baseline、learning-rate sweep、batch-size sweep、四类架构消融、TinyStories 文本生成
- 当前测试：`47 passed, 1 xfailed`；唯一 xfail 是题目预期的 `Tokenizer.encode` 1 MiB 附加内存测试
- 进行中：OpenWebText 编码/LM 与跨语料 tokenizer 对比；完整流水线已启动，实测结果将追加到同一 PR
...
两个 tokenizer 在各自与跨域样本上的 compression ratio，以及 OpenWebText train/validation 编码仍待完成，因此目前不用推测数据补齐定量比较。
...
- 完成双 tokenizer 与跨语料 compression 实验。
