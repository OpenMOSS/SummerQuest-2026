...
- 环境：Python 3.13.11，PyTorch 2.11.0；正确性测试在 CPU 上完成，正式 TinyStories 训练使用 NVIDIA RTX 4060 Ti 16GB
- 已完成：21 个 adapter 对应实现、训练与生成入口、完整 TinyStories 10K tokenizer 与数据编码、TinyStories baseline、learning-rate sweep、batch-size sweep、四类架构消融、TinyStories 文本生成
- 当前测试：`47 passed, 1 xfailed`；唯一 xfail 是题目预期的 `Tokenizer.encode` 1 MiB 附加内存测试
- 进行中：OpenWebText tokenizer/编码/LM 与跨语料 tokenizer 对比；完整流水线已启动，实测结果将追加到同一 PR

本报告只陈述已经完成并有记录支持的结果；尚未运行的实验不会用推测数据补齐。

## 飞书补充文档

- [王惟易 - A1 补充材料](https://fudan-nlp.feishu.cn/wiki/JOHgwE23GiBiLDkTZURc4YL5n0d)（组织内公开，未开启互联网公开访问）
...
OpenWebText 32K tokenizer、两个 tokenizer 在各自与跨域样本上的 compression ratio，以及 OpenWebText train/validation 编码仍待完成，因此目前不做 TinyStories 与 OWT 的虚假比较。
...
- 训练 OpenWebText 32K tokenizer，完成双 tokenizer 与跨语料 compression 实验。
- 编码 OpenWebText train/validation，用与 TinyStories 相同模型架构和 training iterations 训练并生成文本。
...
uv run --with matplotlib python scripts/plot_experiment_logs.py --logs-dir logs/experiments/raw --output-dir assets
cd ../SummerQuest-2026
...
- 完整生成样本：`logs/experiments/generation_tinystories.txt`
- 配置文件：18 份可移植 JSON 配置位于 `submission/configs/`，包括 TinyStories baseline、两个 sweep、六个消融 run，以及待运行的 OpenWebText baseline
- 原始实验日志：17 份 `metrics.jsonl` 位于 `logs/experiments/raw/`
- Loss curves：四张可复现 SVG 位于 `assets/`
