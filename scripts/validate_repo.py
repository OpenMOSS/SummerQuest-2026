...
def validate_template(errors: list[str]) -> None:
    ...
    a3_template = STUDENTS / "_assignment_templates" / "A3" / "README.md"
    if not a3_template.is_file():
        errors.append(
            f"missing A3 assignment template: {a3_template.relative_to(ROOT)}"
        )
...
def validate_a3_submission(assignment: Path, errors: list[str]) -> None:
    readme = assignment / "README.md"
    ...
    for name in A3_DEPENDENCY_FILES:
        ...
        if path.is_file() and path.stat().st_size > A3_MAX_DEPENDENCY_BYTES:
            errors.append(
...
def validate_assignment(student: Path, assignment: Path, errors: list[str]) -> None:
    ...
    if assignment.name == "A0":
        ...
        validate_a3_submission(assignment, errors)
