...
附件大小和禁止文件见 [A2-P 题面](../assignments/A2-P/README.md)与
[A2-K 题面](../assignments/A2-K/README.md)。A3 只要求 Markdown 报告，见
...
`.nsys-rep`、memory snapshot、完整 trace、上游代码、公共 tests 和依赖文件不得同步。
A2-P 的 `results/` 与 `assets/` 公开附件合计不超过 2 MiB；Profile 只提交支撑
结论的关键汇总和裁剪、压缩截图。
...
A3 题面与 API 材料由助教在课程群内发布。创建 Markdown 报告目录：

```bash
python3 scripts/create_assignment.py --name '<同学真名>' --assignment A3
```

A3 的唯一必交文件是公开、脱敏的 `README.md`。如果需要说明本地分析环境，可以附上不超过
256 KiB 的轻量 `requirements.txt` 或 `pyproject.toml`，但不是必交。API 地址、密钥、内部
路径、训练数据、权重、完整服务日志、虚拟环境和自动生成的依赖锁文件不得进入 GitHub。

不要把整个 `assignment1-basics/`、`assignment2-systems/`、公共 tests/fixtures、数据、模型权重或
依赖环境放进 SummerQuest 仓库。

完成后只暂存自己的作业目录，检查差异再提交。A0 使用第一条 `git add`；其他已开放作业
使用对应的第二条：

```bash
git add "students/<同学真名>"
# git add "students/<同学真名>/assignments/<A编号>"
git status
git diff --check
git diff --cached
