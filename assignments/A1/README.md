...
> 以本仓库的实验室版要求为准。公共 starter 基于 Stanford CS336 `assignment1-basics` 的
> `a158843b20107949f1a8d7df1b05cd33b9166712` commit，并保留上游 MIT License。
...
- 公共 starter、依赖锁和测试位于 [`starter/A1`](../../starter/A1/README.md)。
- 上游来源为 [stanford-cs336/assignment1-basics](https://github.com/stanford-cs336/assignment1-basics)。
...
脚手架会创建下面的固定目录。PR 只提交这个个人 A1 目录，不复制公共 tests、fixtures、
依赖文件或整个 starter：
...
导出文件。依赖由公共 `starter/A1/uv.lock` 固定，个人提交中不添加 `pyproject.toml`、
`requirements.txt` 或 lock file。

## 文件规则

- 文件大小限制和日志的具体格式由后续作业批改助教决定并补充。
- GitHub 与飞书的公开范围继续遵循仓库统一的
  [公开性与提交规则](../../docs/submission-rules.md)。

## 代码接口

公共 ABI 是 [`starter/A1/tests/adapters.py`](../../starter/A1/tests/adapters.py) 中的
21 个函数：
...
签名、默认参数、参数语义和返回值以该文件为准。不得改名或删减。starter 保留了原作业
的测试环境；在个人工作副本中按上游方式运行 `uv run pytest`。这只是原作业提供的开发
测试入口。
